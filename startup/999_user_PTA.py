...
class Sample(SampleGISAXS):
    ...
    def align_crazy_v2(self, step=0, reflection_angle=0.12, ROI_size=[10, 180], th_range=0.3, int_threshold=10, verbosity=3):
    ...
    def align_crazy_v3(self, step=0, reflection_angle=0.12, ROI_size=[10, 180], th_range=0.3, int_threshold=10, verbosity=3):
    ...
    def swing_v2(self, step=0, reflection_angle=0.12, ROI_size=[10, 180], th_range=0.3, int_threshold=10, verbosity=3):
        ...
        if step<=2:
            ...
            while abs(pilatus2M.stats4.total.get() - value)/value < 0.1 and ii < 3: 
                ii += 1
                # Find the step-edge
                fastsearch = RE(self.search_plan(motor=smy, step_size=.1, min_step=0.01, target=0.5, intensity=20000, polarity=-1,fastsearch=True, detector_suffix='_stats4_total'))                
                if fastsearch == True:
                    break
                # Find the peak
                # self.thsearch(step_size=0.2, min_step=0.01, target='max', verbosity=verbosity)
                fastsearch = RE(self.search_plan(motor=sth, step_size=.2, min_step=0.01, target='max',fastsearch=True,detector_suffix='_stats4_total'))
                if fastsearch == True:
                    break            
            #last check for height
            # self.ysearch(step_size=0.05, min_step=0.005, intensity=value, target=0.5, verbosity=verbosity, polarity=-1)
    ...
    def crazy_th(self, step=0, reflection_angle=0.12, ROI_size=[10, 180], th_range=0.3, int_threshold=10, verbosity=3):
    ...
    def measureAutonomous(self, runno=0, exposure_time=10, extra=None, max_measurements=600000, prefix='measureAutonomous > ', verbosity=3, **md):
        ...
        for i in range(runno, max_measurements):
            ...
            for icommand, command in enumerate(commands):
                ...
                if not command['measured']:
                    ...
                    self.name = self.name_o + '_Ti{:.1f}'.format(Ti) + '_Tm{:.1f}'.format(Tm) + '_Tf{:.1f}'.format(Tf) 
    ...
    def measureAutonomous_3samples(self, runNo=0, exposure_time=10, extra=None, max_measurements=600000, samplePosNo=0, prefix='measureAutonomous > ', verbosity=3, align=False, **md):
        '''Measure points in a loop, relying on an external queue to specify what
        position to actually measure. If the 'position' is not (x,y) sample coordinates,
        then you will have to add code to do the appropriate coordinate conversion,
        or trigger the right beamline motors/components.'''
        ...
        for i in range(runNo, max_measurements):
            ...
            for icommand, command in enumerate(commands):
                ...
                if not command['measured']:
                    ...
                    # command['position_gpcam'] = command['position']
                    self.name = self.name_o + '_Ti{:.1f}'.format(Ti) + '_Tm{:.1f}'.format(Tm) + '_Tf{:.1f}'.format(Tf) + '_run{}'.format(i) 
                    ...
                    cms.modeMeasurement()
                    self.measureIncidentAngles([0.08, 0.1, 0.12, 0.15, 0.18, 0.2, 0.25], exposure_time=exposure_time, extra='FINAL')
...
RE.md['experiment_alias_directory'] = '/nsls2/data/cms/legacy/xf11bm/data/2023_1/beamline/PTA/'
...
def fake_coordinated_motionr(mtr1, mtr2, delta, step=.1):
    for j in range(int(delta / step)):
        yield from bps.mvr(mtr1, step, mtr2, step)
